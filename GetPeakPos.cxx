...
void GetPeakPos(TString period = "13TeV"){

  TString name = "Pi0Tagging_8TeV_02_04.root";
  if(period.Contains("13TeV")) name = "Pi0Tagging_13TeV_02_04.root";
  ...
  if(period.Contains("13TeV"))gSystem->Exec("mkdir PeakPos13TeV");
  ...
  for(int i = 0; i < vec.size() - 1; ++i){
    ...
    hprojData->Draw();
    fgaus->Draw("same");
    if(period.Contains("13TeV"))Can.SaveAs(Form("PeakPos13TeV/DataBin%i.png", i));
  ...
  for(int i = 0; i < vec.size() - 1; ++i){
    ...
    hprojMC->Draw();
    fgaus->Draw("same");
    if(period.Contains("13TeV"))Can.SaveAs(Form("PeakPos13TeV/MCBin%i.png", i));
  ...
  TF1 funcData("funcData", "[0] + [1]*TMath::Power(x,[2])", 1.4,6);
  TF1 funcMC("funcMC", "[0] + [1]*TMath::Power(x,[2])", 1.4,6);
  grData.Fit(&funcData, "MQR0");
  grMC.Fit(&funcMC, "MQR0");

  TFile fMassPos("fMassPos.root", "Update");
  fMassPos.cd();
  if(period.Contains("13TeV")){
    grData.Write("MassPos13TeV_data", TObject::kOverwrite);
    grMC.Write("MassPos13TeV_MC", TObject::kOverwrite);
    funcData.Write("MassPos13TeV_func", TObject::kOverwrite);
  ...
  grMC.Draw("same");
  if(period.Contains("13TeV")) Can.SaveAs("PeakPos13TeV/PeakPos.png");
  else Can.SaveAs("PeakPos8TeV/PeakPos.png");
